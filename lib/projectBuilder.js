...
var CSBuilder, CopyBuilder, JSBuilder, JSONBuilder, JadeBuilder, StylusBuilder, async, buildFactory, config, exec, fileConfig, fs, logger, path, postBuild, util, walk, _,
...
buildFactory = {
  ...
  get: function(file) {
    return this[path.extname(file)] || this.copy;
  }
};

postBuild = function() {
  var _ref;
  if ((_ref = postBuild.process) != null) {
    _ref.kill();
  }
  return postBuild.process = exec(config.post_build.cmd, function(err, stdout, stderr) {
    console.log(stdout + stderr);
    return postBuild.process = null;
  });
};
...
module.exports = {
  ...
  liveBuild: function(src, cb) {
    return buildFactory.get(src).build(src, true, function(err, file, message) {
      cb(err, file, message);
      if (err || /identical/.test(message)) {
        return;
      }
      if (config.post_build && config.post_build.match.test(src)) {
        return postBuild();
      }
    });
  },
