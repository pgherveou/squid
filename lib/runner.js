(function() {
  var Monitor, argv, buildReady, builder, codeChange, fileFilter, filter, fs, hrLog, hrLogId, killApp, libMonitor, logger, moment, notifier, path, relativeName, restart, root, server, serverScript, spawn, srcMonitor, srvArgs, start, startTime, writeStderr, writeStdout;
  ...
  */
  ...
  if (argv["break"]) srvArgs.push('--debug-brk');
  ...
  */
  ...
  filter = function(f, stat) {
    return stat.isDirectory() || fileFilter(f);
  };
  ...
  libMonitor.once('started', function() {
  ...
  srcMonitor.on('created', function(f) {
    if (fileFilter(f)) return builder.liveBuild(f, codeChange);
  });
