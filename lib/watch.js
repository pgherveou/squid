...
var Monitor, argv, codeChange, fs, hrLog, hrLogId, logger, moment, notifier, path, project, relativeName, root, server, serverScript, spawn, srcMonitor, srvArgs, start, writeStderr, writeStdout, _, _ref;
...
codeChange = function(err, file, newCode) {
  ...
  if ((serverScript && !server) || (server && server.exitCode)) {
    return start("Restarting " + serverScript);
  }
};
