var BuildError, Builder, StylusBuilder, async, logger, nib, path, stylus, _ref,
...
module.exports = StylusBuilder = (function(_super) {
  ...
  StylusBuilder.name = 'StylusBuilder';
  ...
  StylusBuilder.prototype._compile = function(file, code, cb) {
    return stylus(code).set('fileName', file).set('compress', true).define('env', process.env.NODE_ENV || 'development').set('paths', ['public/images', path.dirname(file)]).define('url', stylus.url({
      paths: ['public']
