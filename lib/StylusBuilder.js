...
module.exports = StylusBuilder = (function(_super) {
  ...
  StylusBuilder.prototype._compile = function(file, code, cb) {
    var paths;
    paths = this.config.stylus.paths;
    return stylus(code).set('fileName', file).set('compress', true).define('env', process.env.NODE_ENV || 'development').define('host', os.hostname()).set('paths', paths.concat(path.dirname(file))).define('url', stylus.url({
      paths: this.config.stylus.url
