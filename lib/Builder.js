...
(function() {
  var BuildError, Builder, async, fs, logger, mkdirp, path, _,
  ...
  exports.Builder = Builder = (function() {
    ...
    Builder.prototype.write = function(newCode, src, cb) {
      ...
      return fs.readFile(file, 'utf8', function(err, oldCode) {
        if (newCode === oldCode) {
          return cb(null, file, "identical files");
        }
        ...
        return mkdirp(path.dirname(file), 0x1ed, function(err) {
          ...
          return fs.writeFile(file, newCode, function(err) {
            ...
            return _this.config.clone.forEach(function(clone) {
              var map;
              if (clone.match.test(src)) {
                file = _this.buildPath(src, path.resolve(clone.to));
                if (map = clone.map) {
                  file = file.replace(map.from, map.to);
                }
                return mkdirp(path.dirname(file), 0x1ed, function(err) {
                  if (err) {
                    return logger.error("Error cloning dir to ", file);
                  }
                  return fs.writeFile(file, newCode, function(err) {
                    if (err) {
                      return logger.error("Error cloning file to ", file);
                    }
                  });
                });
              }
